...
export default function HitTrackerTable() {
  ...
  const getTrendIcon = (current: number, previous: number) => {
    if (!previous || current === previous) return <Minus className="w-3 h-3 text-gray-600" />
    ...
    <div className="space-y-6">
      ...
      <div className="flex space-x-2 mb-4">
        {machineData.map((machine) => (
          ...
          >
            ...
            <div className="text-xs text-gray-700">Target: {machine.target}/hr</div>
          </button>
        ))}
      </div>
              ...
              <tr>
                ...
                {week?.days.map((day) => (
                  <th key={day.date} className="px-4 py-3 text-center text-xs font-medium text-gray-700 uppercase tracking-wider">
                    <div>{day.dayName}</div>
                    <div className="text-gray-700 font-normal">{day.date.slice(5)}</div>
                  </th>
                ))}
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="bg-white rounded-lg shadow p-4">
          ...
          <div className="text-xs text-gray-700">Target: {(currentMachine?.target * (week?.weeklyHours || 0))?.toLocaleString()}</div>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          ...
          <div className="text-xs text-gray-700">Active Production</div>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          ...
          <div className="text-xs text-gray-700">Target: 100%</div>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          <div className="text-sm text-gray-600">Data Source</div>
          <div className="text-2xl font-bold text-gray-900">Database</div>
          <div className="text-xs text-gray-700">Real production data</div>
        </div>
      </div>
    </div>
  )
